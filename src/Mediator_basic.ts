...
interface IRequestHandler<U extends IRequest<T>, T> {
...
class CommandA implements IRequest<string>, ICommand {
...
class CommandAHandler implements IRequestHandler<CommandA, string> {
  
  handle(request: CommandA): string {
    return "what's up";
  }
}
